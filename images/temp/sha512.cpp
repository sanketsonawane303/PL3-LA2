 ...
 * @param input message string used as an input to the SHA512 algorithm, must be
 * < size_t bits
 */
std::string SHA512::hash(const std::string input) {
  ...
  uint64* h = new uint64[HASH_LEN];  // buffer holding the message digest
                                     // (512-bit = 8 64-bit words)
...
uint64** SHA512::preprocess(const unsigned char* input, size_t& nBuffer) {
  ...
  size_t k = (896 - 1 - l) % MESSAGE_BLOCK_SIZE;  // length of zero bit padding
                                                  // (l + 1 + k = 896 mod 1024)
  ...
  appendLen(l, buffer[nBuffer - 1][SEQUENCE_LEN - 1],
...
int main(int argc, char* argv[]) {
  ...
  std::cout << "SHA512: " << sha512.hash(plaintext) << std::endl;
